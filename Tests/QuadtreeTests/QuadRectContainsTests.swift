...
final class QuadRectContainsTests: XCTestCase {
    func testTopLeadingAnchor() {
        ...
        XCTAssertTrue(boundary.contains(point: .init(x: 0, y: 9)))
        XCTAssertTrue(boundary.contains(point: .init(x: 9, y: 0)))
        XCTAssertTrue(boundary.contains(point: .init(x: 9, y: 9)))
    }
